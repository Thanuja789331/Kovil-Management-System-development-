...
-- Safer import on WAMP/phpMyAdmin (re-enabled at end of core tables)
...
-- Bookings table (FK + generated column added below for WAMP/MySQL compatibility)
CREATE TABLE IF NOT EXISTS bookings (
    ...
    reminder_24_hour_email_sent TINYINT(1) DEFAULT 0,
    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    UNIQUE KEY uq_booking_reference (booking_reference),
    INDEX idx_user_id (user_id),
    INDEX idx_schedule_id (schedule_id),
    INDEX idx_bookings_user_status (user_id, status),
    INDEX idx_bookings_schedule_status (schedule_id, status)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;

-- Parent tables must be InnoDB with matching INT keys (fixes #1215 on some WAMP installs)
ALTER TABLE users ENGINE=InnoDB;
ALTER TABLE pooja_schedule ENGINE=InnoDB;

-- Add booking foreign keys (safe to re-run - will error if already exists, which is fine)
ALTER TABLE bookings ADD CONSTRAINT fk_bookings_schedule FOREIGN KEY (schedule_id) REFERENCES pooja_schedule(id) ON DELETE CASCADE;
ALTER TABLE bookings ADD CONSTRAINT fk_bookings_user FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE;

-- Note: "one confirmed booking per schedule" is enforced in app code (Booking.php)
-- using transactions. Generated-column unique indexes cause #1215 on some WAMP/MariaDB builds.
...
-- Pooja Requests table (for devotees to request poojas on preferred dates)
...
-- Sample Donations
INSERT INTO donations (donor_name, amount, purpose, payment_method, donation_reference, payment_status) VALUES
...
-- Booking/donation integrity indexes are defined in CREATE TABLE for fresh installs.
