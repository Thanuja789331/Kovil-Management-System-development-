...
class Duty {
    ...
    /**
     * Check if priest is already assigned to schedule.
     */
    public function isAlreadyAssigned($priest_id, $schedule_id) {
        $stmt = $this->conn->prepare("SELECT id FROM priest_duties WHERE priest_id = ? AND schedule_id = ?");
        $stmt->bind_param("ii", $priest_id, $schedule_id);
        $stmt->execute();
