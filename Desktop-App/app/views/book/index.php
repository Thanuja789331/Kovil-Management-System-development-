    ...
    <div class="w-full max-w-3xl">
        ...
        <div class="glass-card p-8 card-hover">
            ...
            <form method="POST" id="bookingForm" class="space-y-6">
                ...
                <div>
                    ...
                    >
                    <p class="text-xs text-gray-500 mt-1">Required only when SMS or Both is selected</p>
                </div>
                ...
                <div>
                    <label class="block text-sm font-semibold text-gray-700 mb-3">Send Confirmation To <span class="text-red-500">*</span></label>
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
                        <label class="flex items-center space-x-2 p-3 border border-gray-300 rounded-lg bg-white/70">
                            <input type="radio" name="notification_preference" value="sms" class="text-secondary-600" required>
                            <span class="text-sm font-medium text-gray-700">SMS only</span>
                        </label>
                        <label class="flex items-center space-x-2 p-3 border border-gray-300 rounded-lg bg-white/70">
                            <input type="radio" name="notification_preference" value="email" class="text-secondary-600">
                            <span class="text-sm font-medium text-gray-700">Email only</span>
                        </label>
                        <label class="flex items-center space-x-2 p-3 border border-gray-300 rounded-lg bg-white/70">
                            <input type="radio" name="notification_preference" value="both" class="text-secondary-600" checked>
                            <span class="text-sm font-medium text-gray-700">Both SMS and Email</span>
                        </label>
                    </div>
                    <p class="text-xs text-gray-500 mt-1">Your receipt will include pooja details and required bring-items.</p>
                </div>
                ...
                <div class="flex items-start space-x-3">
                    ...
                    <label for="confirm" class="text-sm text-gray-700">
                        I confirm that I will arrive 15 minutes before the scheduled time and I agree to receive booking notifications on my selected channel.
                    </label>
                </div>
        ...
        <div class="mt-6 bg-blue-50 p-6 rounded-xl border-2 border-blue-200">
            ...
            <ul class="space-y-2 text-sm text-blue-700">
                ...
                <li class="flex items-start">
                    <span class="mr-2">✓</span>
                    <span>Confirmation will be sent based on your selected notification channel</span>
                </li>
                <li class="flex items-start">
                    <span class="mr-2">✓</span>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const preferenceInputs = document.querySelectorAll('input[name="notification_preference"]');

    function syncPhoneRequirement() {
        if (!phoneInput) return;
        const selected = document.querySelector('input[name="notification_preference"]:checked');
        const pref = selected ? selected.value : 'both';
        const mustHavePhone = pref === 'sms' || pref === 'both';
        phoneInput.required = mustHavePhone;
    }
    preferenceInputs.forEach(input => input.addEventListener('change', syncPhoneRequirement));
