...
$approvedDevotees = $data['approvedDevotees'] ?? [];
...
$currentBookedUserId = intval($schedule['booking_user_id'] ?? 0);
?>
<div class="space-y-6">
    <!-- Back Button -->
        ...
        <form action="?url=schedule&action=update" method="POST" class="space-y-6">
            ...
            <div>
                ...
                <input 
                    ...
                    name="pooja_name" 
                    required
                    value="<?= htmlspecialchars($schedule['pooja_name'] ?? '') ?>"
            ...
            <div>
                ...
                <input 
                    ...
                    name="pooja_date" 
                    required
                    value="<?= htmlspecialchars($schedule['pooja_date'] ?? '') ?>"
                    class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-primary-500 focus:ring-4 focus:ring-primary-200 transition-all outline-none"
                >
            </div>
            ...
            <div>
                ...
                >
                    ...
                    <option value="06:00:00" <?= ($schedule['time_slot'] ?? '') == '06:00:00' ? 'selected' : '' ?>>Morning 6:00 AM</option>
                    <option value="07:00:00" <?= ($schedule['time_slot'] ?? '') == '07:00:00' ? 'selected' : '' ?>>Morning 7:00 AM</option>
                    <option value="08:00:00" <?= ($schedule['time_slot'] ?? '') == '08:00:00' ? 'selected' : '' ?>>Morning 8:00 AM</option>
                    <option value="09:00:00" <?= ($schedule['time_slot'] ?? '') == '09:00:00' ? 'selected' : '' ?>>Morning 9:00 AM</option>
                    <option value="10:00:00" <?= ($schedule['time_slot'] ?? '') == '10:00:00' ? 'selected' : '' ?>>Morning 10:00 AM</option>
                    <option value="11:00:00" <?= ($schedule['time_slot'] ?? '') == '11:00:00' ? 'selected' : '' ?>>Morning 11:00 AM</option>
                    <option value="16:00:00" <?= ($schedule['time_slot'] ?? '') == '16:00:00' ? 'selected' : '' ?>>Evening 4:00 PM</option>
                    <option value="17:00:00" <?= ($schedule['time_slot'] ?? '') == '17:00:00' ? 'selected' : '' ?>>Evening 5:00 PM</option>
                    <option value="18:00:00" <?= ($schedule['time_slot'] ?? '') == '18:00:00' ? 'selected' : '' ?>>Evening 6:00 PM</option>
                </select>
            </div>
            ...
            <div>
                ...
                ><?= htmlspecialchars($schedule['description'] ?? '') ?></textarea>
            </div>

            <div class="bg-blue-50 border border-blue-200 rounded-xl p-5">
                <h3 class="font-bold text-blue-900 mb-3">Booking Management (Admin)</h3>
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        <label for="booking_status" class="block text-sm font-semibold text-gray-700 mb-2">Booking Status</label>
                        <select id="booking_status" name="booking_status" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-primary-500 outline-none bg-white">
                            <option value="available" <?= !$isBooked ? 'selected' : '' ?>>Available</option>
                            <option value="booked" <?= $isBooked ? 'selected' : '' ?>>Booked</option>
                        </select>
                        <?php if ($isBooked): ?>
                            <p class="text-xs text-gray-600 mt-1">Currently booked by: <strong><?= htmlspecialchars($schedule['booked_by_name'] ?? 'Unknown') ?></strong></p>
                        <?php endif; ?>
                    </div>
                    <div>
                        <label for="assigned_devotee_id" class="block text-sm font-semibold text-gray-700 mb-2">Assign/Change Devotee</label>
                        <select id="assigned_devotee_id" name="assigned_devotee_id" class="w-full px-4 py-3 border-2 border-gray-200 rounded-xl focus:border-primary-500 outline-none bg-white">
                            <option value="">Select devotee</option>
                            <?php foreach ($approvedDevotees as $devotee): ?>
                                <option value="<?= (int) $devotee['id'] ?>" <?= $currentBookedUserId === (int) $devotee['id'] ? 'selected' : '' ?>>
                                    <?= htmlspecialchars($devotee['name']) ?> (<?= htmlspecialchars($devotee['email']) ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                        <p class="text-xs text-gray-600 mt-1">If status is Booked, selected devotee will be assigned.</p>
                    </div>
                </div>
            </div>
