<div class="min-h-[calc(100vh-8rem)] flex items-center justify-center py-12 px-4">
    <div class="max-w-3xl mx-auto bg-white p-8 rounded-xl shadow-lg">
        ...
        <div class="space-y-3">
            ...
            <a href="?url=confirmation&action=download-pdf&booking_id=<?= urlencode($data['id']) ?>"
               class="block w-full bg-gradient-to-r from-indigo-600 to-indigo-700 hover:from-indigo-700 hover:to-indigo-800 text-white font-bold py-3 px-4 rounded-lg transition transform hover:scale-105 text-center">
                Download Booking PDF Receipt
            </a>
            <?php endif; ?>
            
            <?php if(isset($data['id'])): ?>
            <div class="bg-green-50 border border-green-200 rounded-lg p-4">
                <label for="notificationChannel" class="block text-sm font-semibold text-gray-700 mb-2">Send Confirmation Receipt To</label>
                <div class="flex flex-col sm:flex-row gap-3">
                    <select id="notificationChannel" class="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-gray-800 bg-white">
                        <option value="sms">SMS only</option>
                        <option value="email">Email only</option>
                        <option value="both" selected>Both SMS and Email</option>
                    </select>
                    <button onclick="sendConfirmationNotification('<?= htmlspecialchars($data['id']) ?>')"
                            class="bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-bold py-2 px-4 rounded-lg transition transform hover:scale-105 shadow-lg">
                        Send Confirmation
                    </button>
                </div>
            </div>
...
<script>
function sendConfirmationNotification(bookingId) {
    const statusDiv = document.getElementById('sms-status');
    const channel = document.getElementById('notificationChannel')?.value || 'both';
    
    // Show loading state
    statusDiv.classList.remove('hidden');
    statusDiv.className = 'mt-4 p-4 rounded-lg bg-yellow-600/20 text-yellow-200';
    statusDiv.innerHTML = '⏳ Sending confirmation via ' + channel.toUpperCase() + '...';
    ...
    fetch('?url=confirmation&action=send-sms', {
        ...
        body: `booking_id=${bookingId}&notification_channel=${encodeURIComponent(channel)}`
    })
