...
<div class="space-y-6">
    ...
    <div class="text-center mb-8">
        ...
        <div class="mt-4">
            <a href="?url=festival&action=add" class="inline-block bg-gradient-to-r from-primary-700 to-primary-800 hover:from-primary-600 hover:to-primary-700 text-white font-semibold py-2 px-5 rounded-xl shadow-md transition-all duration-200">
                Add Festival
            </a>
        </div>
        <?php endif; ?>
    ...
    <div class="glass-card p-4 mb-6">
        <form method="GET" class="flex flex-col md:flex-row md:items-end gap-3">
            <input type="hidden" name="url" value="festival">
            <div class="flex-1">
                ...
                <input
                    ...
                    class="w-full px-4 py-2.5 border-2 border-gray-200 rounded-xl bg-white/80 text-gray-800 focus:border-primary-500 focus:outline-none"
                >
            </div>
            <button
                type="submit"
                class="bg-gradient-to-r from-primary-700 to-primary-800 hover:from-primary-600 hover:to-primary-700 text-white px-5 py-2.5 rounded-xl font-semibold transition-all duration-200 shadow-lg"
            >
                Filter
            </button>
            <a
        ...
        <?php if(!empty($festivals)): ?>
            ...
            <div class="glass-card p-6 card-hover border-l-4 border-primary-600">
                ...
                <div class="mt-4 pt-4 border-t border-gray-200 flex justify-end gap-3">
                    <?php if(isset($_SESSION['user']) && $_SESSION['user']['role'] === 'management' && !empty($festival['editable']) && !empty($festival['id'])): ?>
                        <a href="?url=festival&action=edit&id=<?= $festival['id'] ?>" class="text-yellow-600 hover:text-yellow-700 font-semibold text-sm transition-colors">
                            Edit
                        </a>
                    <?php endif; ?>
                    <?php
                    $detailsUrl = '?url=festival&action=details';
                    if (!empty($festival['id'])) {
                        $detailsUrl .= '&id=' . urlencode((string) $festival['id']);
                    } else {
                        $detailsUrl .= '&name=' . urlencode((string) ($festival['name'] ?? ''));
                        $detailsUrl .= '&date=' . urlencode((string) ($festival['date'] ?? ''));
                        $detailsUrl .= '&description=' . urlencode((string) ($festival['description'] ?? ''));
                        $detailsUrl .= '&category=' . urlencode((string) ($festival['category'] ?? 'Event'));
                    }
                    ?>
                    <a href="<?= $detailsUrl ?>" class="text-primary-600 hover:text-primary-700 font-semibold text-sm transition-colors">
                        Learn More ->
                    </a>
                </div>
            </div>
